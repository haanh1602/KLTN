...
public class UbhBullet : UbhMonoBehaviour
{
    ...
    public void ResetMtranform(Transform homingTarget2,float speed,float homingspeed,float timeMove,float speedEnd,float speedAngleEnd)
    {
        m_speed = speed;
        m_homingAngleSpeed = homingspeed;
        m_homingTarget = homingTarget2;
        StartCoroutine(ResetMtranformEndIE(timeMove, speedEnd, speedAngleEnd));
    }
    IEnumerator ResetMtranformEndIE(float timeMove,float speedEnd,float speedAngleEnd)
    {
        yield return new WaitForSeconds(timeMove);
        m_speed = speedEnd;
        m_homingAngleSpeed = speedAngleEnd;
        //m_homingTarget = PlaneIngameManager.current.CurrentTransformPlayer;
        yield return null;
    }
    public void ResetMtranformEnd(Transform homingTarget2, float speed, float homingspeed)
    {
        m_speed = speed;
        m_homingAngleSpeed = homingspeed;
        m_homingTarget = homingTarget2;
    ...
    public void UpdateMove(float deltaTime)
    {
        ...
        if (m_useAutoRelease && m_autoReleaseTime > 0f)
        {
            ...
            {
                // Release
                //SgkBullet sgkBullet=gameObject.GetComponent<>() 
                UbhObjectPool.instance.ReleaseBullet(this);
                return;
            }
        }
        ...
        {
            //TuanManager.Log("Gameobject: " + gameObject.name);
            ...
            if (0f < m_waveSpeed && 0f < m_waveRangeSize)
            {
                float waveAngle;
                if (!stCos)
                    waveAngle = m_angle + (m_waveRangeSize / 2f * Mathf.Sin(m_selfFrameCnt * m_waveSpeed / 100f));
                else
                    waveAngle = m_angle + (-m_waveRangeSize / 2f * Mathf.Cos(m_selfFrameCnt * m_waveSpeed / 100f));
